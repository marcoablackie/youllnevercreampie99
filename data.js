/*
 * NBA 2K26 Jumpshot Creator - SHOTS DATA
 * Scraped from GameRant + NBA2KW (Jun 2026). Unlock requirements only.
 * Go-To timings: NBA2KLab moving-jumpers (public) + estimates for unrated players.
 * Types: jump_shot, go_to, dribble_pullup, post_fade
 */
const SHOTS = [
  {
    "name": "Anthony Davis",
    "type": "dribble_pullup",
    "rating": 78,
    "height": "6'10 or higher"
  },
  {
    "name": "Anthony Edwards",
    "type": "dribble_pullup",
    "rating": 80,
    "height": "Under 6'10"
  },
  {
    "name": "Bam Adebayo",
    "type": "dribble_pullup",
    "rating": 81,
    "height": "6'5 or higher"
  },
  {
    "name": "Bogdan Bogdanovic",
    "type": "dribble_pullup",
    "rating": 86,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Bradley Beal",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Under 6'5"
  },
  {
    "name": "Brandon Ingram",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Buddy Hield",
    "type": "dribble_pullup",
    "rating": 72,
    "height": "Under 6'5"
  },
  {
    "name": "Cade Cunningham",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Chet Holmgren",
    "type": "dribble_pullup",
    "rating": 76,
    "height": "6'10 or higher"
  },
  {
    "name": "Dante Exum",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Darius Garland",
    "type": "dribble_pullup",
    "rating": 84,
    "height": "Under 6'5"
  },
  {
    "name": "De'Aaron Fox",
    "type": "dribble_pullup",
    "rating": 78,
    "height": "Under 6'5"
  },
  {
    "name": "DeMar DeRozan",
    "type": "dribble_pullup",
    "rating": 82,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Desmond Bane",
    "type": "dribble_pullup",
    "rating": 86,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Devin Booker",
    "type": "dribble_pullup",
    "rating": 86,
    "height": "Under 6'10"
  },
  {
    "name": "Donte DiVincenzo",
    "type": "dribble_pullup",
    "rating": 82,
    "height": "Under 6'5"
  },
  {
    "name": "Eric Gordon",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "Under 6'10"
  },
  {
    "name": "Gary Harris",
    "type": "dribble_pullup",
    "rating": 76,
    "height": "Under 6'5"
  },
  {
    "name": "Giannis Antetokounmpo",
    "type": "dribble_pullup",
    "rating": 71,
    "height": "6'10 or higher"
  },
  {
    "name": "Gordon Hayward",
    "type": "dribble_pullup",
    "rating": 82,
    "height": "Under 6'10"
  },
  {
    "name": "Harrison Barnes",
    "type": "dribble_pullup",
    "rating": 85,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Jalen Brunson",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Under 6'5"
  },
  {
    "name": "Jalen Green",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "Under 6'10"
  },
  {
    "name": "James Harden",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Under 6'10"
  },
  {
    "name": "Jaren Jackson Jr.",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "6'10 or higher"
  },
  {
    "name": "Jaylen Brown",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Jimmy Butler",
    "type": "dribble_pullup",
    "rating": 77,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "John Collins",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "6'5 or higher"
  },
  {
    "name": "Jordan Clarkson",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Under 6'5"
  },
  {
    "name": "Jrue Holiday",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Under 6'10"
  },
  {
    "name": "Kentavious Caldwell-Pope",
    "type": "dribble_pullup",
    "rating": 86,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Keon Ellis",
    "type": "dribble_pullup",
    "rating": 81,
    "height": "Under 6'5"
  },
  {
    "name": "Kevin Huerter",
    "type": "dribble_pullup",
    "rating": 75,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Keyonte George",
    "type": "dribble_pullup",
    "rating": 76,
    "height": "Under 6'5"
  },
  {
    "name": "Kobe Bryant",
    "type": "dribble_pullup",
    "rating": 86,
    "height": "Under 6'10"
  },
  {
    "name": "Kyrie Irving",
    "type": "dribble_pullup",
    "rating": 88,
    "height": "Under 6'5"
  },
  {
    "name": "LaMelo Ball",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "Under 6'10"
  },
  {
    "name": "LeBron James",
    "type": "dribble_pullup",
    "rating": 81,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Luka Doncic",
    "type": "dribble_pullup",
    "rating": 80,
    "height": "Under 6'10"
  },
  {
    "name": "Malaki Branham",
    "type": "dribble_pullup",
    "rating": 78,
    "height": "Under 6'5"
  },
  {
    "name": "Malik Beasley",
    "type": "dribble_pullup",
    "rating": 76,
    "height": "Under 6'5"
  },
  {
    "name": "Markelle Fultz",
    "type": "dribble_pullup",
    "rating": 66,
    "height": "Under 6'5"
  },
  {
    "name": "Mikal Bridges",
    "type": "dribble_pullup",
    "rating": 82,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Mike Conley",
    "type": "dribble_pullup",
    "rating": 77,
    "height": "Under 6'5"
  },
  {
    "name": "Paolo Banchero",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "6'10 or higher"
  },
  {
    "name": "Paul George",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Reggie Jackson",
    "type": "dribble_pullup",
    "rating": 81,
    "height": "Under 6'5"
  },
  {
    "name": "Rui Hachimura",
    "type": "dribble_pullup",
    "rating": 82,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Scoot Henderson",
    "type": "dribble_pullup",
    "rating": 78,
    "height": "Under 6'5"
  },
  {
    "name": "Shai Gilgeous-Alexander",
    "type": "dribble_pullup",
    "rating": 86,
    "height": "Under 6'5"
  },
  {
    "name": "Spencer Dinwiddie",
    "type": "dribble_pullup",
    "rating": 83,
    "height": "Under 6'10"
  },
  {
    "name": "Stephen Curry",
    "type": "dribble_pullup",
    "rating": 92,
    "height": "Under 6'5"
  },
  {
    "name": "Taylor Hendricks",
    "type": "dribble_pullup",
    "rating": 77,
    "height": "Under 6'5"
  },
  {
    "name": "Tim Hardaway Jr.",
    "type": "dribble_pullup",
    "rating": 79,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Trey Burke",
    "type": "dribble_pullup",
    "rating": 80,
    "height": "Under 6'5"
  },
  {
    "name": "Tyrese Haliburton",
    "type": "dribble_pullup",
    "rating": 82,
    "height": "Under 6'10"
  },
  {
    "name": "Anthony Davis",
    "type": "go_to",
    "rating": 78,
    "height": "6'10 or higher"
  },
  {
    "name": "Anthony Edwards",
    "type": "go_to",
    "rating": 80,
    "height": "Under 6'5"
  },
  {
    "name": "Austin Reaves",
    "type": "go_to",
    "rating": 82,
    "height": "Under 6'10"
  },
  {
    "name": "Bam Adebayo",
    "type": "go_to",
    "rating": 81,
    "height": "6'5 or higher"
  },
  {
    "name": "Bennedict Mathurin",
    "type": "go_to",
    "rating": 75,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Bojan Bogdanovic",
    "type": "go_to",
    "rating": 77,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Bradley Beal",
    "type": "go_to",
    "rating": 83,
    "height": "Under 6'5"
  },
  {
    "name": "Buddy Hield",
    "type": "go_to",
    "rating": 72,
    "height": "Under 6'5"
  },
  {
    "name": "Carmelo Anthony",
    "type": "go_to",
    "rating": 84,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Chris Paul",
    "type": "go_to",
    "rating": 85,
    "height": "Under 6'5"
  },
  {
    "name": "CJ McCollum",
    "type": "go_to",
    "rating": 79,
    "height": "Under 6'5"
  },
  {
    "name": "Damian Lillard",
    "type": "go_to",
    "rating": 82,
    "height": "Under 6'5"
  },
  {
    "name": "Darius Garland",
    "type": "go_to",
    "rating": 84,
    "height": "Under 6'5"
  },
  {
    "name": "De'Aaron Fox",
    "type": "go_to",
    "rating": 78,
    "height": "Under 6'5"
  },
  {
    "name": "Deandre Ayton",
    "type": "go_to",
    "rating": 75,
    "height": "6'10 or higher"
  },
  {
    "name": "DeMar DeRozan",
    "type": "go_to",
    "rating": 82,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Devin Booker",
    "type": "go_to",
    "rating": 86,
    "height": "Under 6'10"
  },
  {
    "name": "Dillon Brooks",
    "type": "go_to",
    "rating": 85,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Dirk Nowitzki",
    "type": "go_to",
    "rating": 78,
    "height": "6'5 or higher"
  },
  {
    "name": "Donovan Mitchell",
    "type": "go_to",
    "rating": 88,
    "height": "Under 6'5"
  },
  {
    "name": "Eric Gordon",
    "type": "go_to",
    "rating": 79,
    "height": "Under 6'5"
  },
  {
    "name": "Harrison Barnes",
    "type": "go_to",
    "rating": 85,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Ja Morant",
    "type": "go_to",
    "rating": 85,
    "height": "Under 6'5"
  },
  {
    "name": "Jaden Hardy",
    "type": "go_to",
    "rating": 77,
    "height": "Under 6'5"
  },
  {
    "name": "Jalen Green",
    "type": "go_to",
    "rating": 79,
    "height": "Under 6'10"
  },
  {
    "name": "Jalen Suggs",
    "type": "go_to",
    "rating": 79,
    "height": "Under 6'10"
  },
  {
    "name": "Jamal Murray",
    "type": "go_to",
    "rating": 83,
    "height": "Under 6'5"
  },
  {
    "name": "James Harden",
    "type": "go_to",
    "rating": 83,
    "height": "Under 6'10"
  },
  {
    "name": "Jayson Tatum",
    "type": "go_to",
    "rating": 82,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Jimmy Butler",
    "type": "go_to",
    "rating": 77,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Joel Embiid",
    "type": "go_to",
    "rating": 77,
    "height": "6'10 or higher"
  },
  {
    "name": "Johnny Juzang",
    "type": "go_to",
    "rating": 85,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Jonathan Kuminga",
    "type": "go_to",
    "rating": 71,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Julius Randle",
    "type": "go_to",
    "rating": 83,
    "height": "6'5 or higher"
  },
  {
    "name": "Keon Ellis",
    "type": "go_to",
    "rating": 71,
    "height": "Under 6'5"
  },
  {
    "name": "Kevin Durant",
    "type": "go_to",
    "rating": 87,
    "height": "6'5 or higher"
  },
  {
    "name": "Kevin Huerter",
    "type": "go_to",
    "rating": 75,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Khris Middleton",
    "type": "go_to",
    "rating": 82,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Klay Thompson",
    "type": "go_to",
    "rating": 82,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Kyle Kuzma",
    "type": "go_to",
    "rating": 68,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Kyrie Irving",
    "type": "go_to",
    "rating": 88,
    "height": "Under 6'5"
  },
  {
    "name": "LaMelo Ball",
    "type": "go_to",
    "rating": 79,
    "height": "Under 6'10"
  },
  {
    "name": "LeBron James",
    "type": "go_to",
    "rating": 81,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Luka Doncic",
    "type": "go_to",
    "rating": 80,
    "height": "Under 6'10"
  },
  {
    "name": "Malik Monk",
    "type": "go_to",
    "rating": 83,
    "height": "Under 6'5"
  },
  {
    "name": "Miles McBride",
    "type": "go_to",
    "rating": 85,
    "height": "Under 6'5"
  },
  {
    "name": "Nikola Jokic",
    "type": "go_to",
    "rating": 84,
    "height": "6'5 or higher"
  },
  {
    "name": "Paolo Banchero",
    "type": "go_to",
    "rating": 79,
    "height": "6'5 or higher"
  },
  {
    "name": "Pascal Siakam",
    "type": "go_to",
    "rating": 81,
    "height": "6'5 or higher"
  },
  {
    "name": "Paul George",
    "type": "go_to",
    "rating": 83,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Reggie Jackson",
    "type": "go_to",
    "rating": 81,
    "height": "Under 6'5"
  },
  {
    "name": "Rui Hachimura",
    "type": "go_to",
    "rating": 82,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Scoot Henderson",
    "type": "go_to",
    "rating": 78,
    "height": "Under 6'5"
  },
  {
    "name": "Shai Gilgeous-Alexander",
    "type": "go_to",
    "rating": 86,
    "height": "Under 6'10"
  },
  {
    "name": "Stephen Curry",
    "type": "go_to",
    "rating": 92,
    "height": "Under 6'5"
  },
  {
    "name": "Terry Rozier III",
    "type": "go_to",
    "rating": 77,
    "height": "Under 6'5"
  },
  {
    "name": "Trae Young",
    "type": "go_to",
    "rating": 82,
    "height": "Under 6'5"
  },
  {
    "name": "Tyrese Haliburton",
    "type": "go_to",
    "rating": 82,
    "height": "Under 6'10"
  },
  {
    "name": "Tyrese Maxey",
    "type": "go_to",
    "rating": 82,
    "height": "Under 6'5"
  },
  {
    "name": "Zach LaVine",
    "type": "go_to",
    "rating": 83,
    "height": "Under 6'10"
  },
  {
    "name": "Zion Williamson",
    "type": "go_to",
    "rating": 70,
    "height": "6'5 or higher"
  },
  {
    "name": "A.J. Green",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "A.J. Lawson",
    "type": "jump_shot",
    "rating": 70,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Aaron Gordon",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Aaron Holiday",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Aaron Nesmith",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Aaron Wiggins",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ace Bailey",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Adam Flagler",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Adem Bona",
    "type": "jump_shot",
    "rating": 41,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "AJ Johnson",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ajay Mitchell",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Al Horford",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Alec Burks",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Alex Caruso",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Alex Len",
    "type": "jump_shot",
    "rating": 57,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Alexandre Sarr",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Allen Iverson",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Alondes Williams",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Alperen Sengun",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Amar'e Stoudemire",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Amen Thompson",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Andre Igoudala",
    "type": "jump_shot",
    "rating": 70,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Andrew Nembhard",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Andrew Wiggins",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Anfernee Simons",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Anthony Black",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Anthony Davis",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Anthony Edwards",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Anthony Gill",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Antonio Reeves",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Ausar Thompson",
    "type": "jump_shot",
    "rating": 61,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Austin Reaves",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Austin Rivers",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Ayo Dosunmu",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Bam Adebayo",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Baylor Scheierman",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Beluba",
    "type": "jump_shot",
    "rating": 88,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Ben Saraf",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ben Sheppard",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ben Simmons",
    "type": "jump_shot",
    "rating": 51,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Bennedict Mathurin",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Bilal Coulibaly",
    "type": "jump_shot",
    "rating": 64,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Bill Laimbeer",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Bismack Biyombo",
    "type": "jump_shot",
    "rating": 47,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Blake Griffin",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Blake Wesley",
    "type": "jump_shot",
    "rating": 65,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Bobby Portis Jr.",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Bobi Klintman",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Bogdan Bogdanovic",
    "type": "jump_shot",
    "rating": 87,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Bojan Bogdanovic",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Bol Bol",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Bradley Beal",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Brandin Podziemski",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Brandon Boston Jr.",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Brandon Clarke",
    "type": "jump_shot",
    "rating": 64,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Brandon Ingram",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Brandon Miller",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Brandon Williams",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Brice Sensabaugh",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Bronny James Jr.",
    "type": "jump_shot",
    "rating": 63,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Brook Lopez",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Bruce Brown",
    "type": "jump_shot",
    "rating": 69,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Bruno Fernando",
    "type": "jump_shot",
    "rating": 51,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Bryce McGowens",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Buddy Hield",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Cade Cunningham",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Caleb Houstan",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Caleb Martin",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cam Christie",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cam Spencer",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Cam Whitmore",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cameron Johnson",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cameron Payne",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Cameron Reddish",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cameron Thomas",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Caris LeVert",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Carlton Carrington",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Carmelo Anthony",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Carter Bryant",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cason Wallace",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Cedric Coward",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Charles Bassey",
    "type": "jump_shot",
    "rating": 63,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Charles Oakley",
    "type": "jump_shot",
    "rating": 70,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Chauncey Billups",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Chet Holmgren",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Chris Boucher",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Chris Brickley",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "Chris Duarte",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Chris Mullin",
    "type": "jump_shot",
    "rating": 88,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Chris Paul",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Chris Webber",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Christian Braun",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Christian Wood",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Chuma Okeke",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "CJ McCollum",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Clint Capela",
    "type": "jump_shot",
    "rating": 47,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Clyde Drexler",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Coby White",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Cody Martin",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cody Williams",
    "type": "jump_shot",
    "rating": 56,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cody Zeller",
    "type": "jump_shot",
    "rating": 44,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Colby Jones",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cole Anthony",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Cole Swider",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Collin Gillespie",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Collin Murray-Boyles",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Collin Sexton",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Cooper Flagg",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Corey Kispert",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Cory Joseph",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "D'Angelo Russell",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Daishen Nix",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Dalano Banton",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Dalen Terry",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Dalton Knecht",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Damian Jones",
    "type": "jump_shot",
    "rating": 41,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Damian Lillard",
    "type": "jump_shot",
    "rating": 87,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Damion Baugh",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Damion Lee",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Dan Rue",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "Daniel Gafford",
    "type": "jump_shot",
    "rating": 40,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Daniel Theis",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Danilo Gallinari",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Danny Green",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Danny Wolf",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Dante Exum",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Danuel House",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Dario Saric",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Dariq Whitehead",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Darius Garland",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Daron Holmes II",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Darren Collison",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "David Robinson",
    "type": "jump_shot",
    "rating": 46,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "David Roddy",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "David West",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Davion Mitchell",
    "type": "jump_shot",
    "rating": 61,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Davis Bertans",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Day'Ron Sharpe",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "De'Aaron Fox",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "De'Andre Hunter",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "De'Anthony Melton",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Dean Wade",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "DeAndre Ayton",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "DeAndre Jordan",
    "type": "jump_shot",
    "rating": 42,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Dejounte Murray",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Delon Wright",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "DeMar DeRozan",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "DeMarcus Cousins",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Deni Avdija",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Dennis Schroder",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Dereck Lively II",
    "type": "jump_shot",
    "rating": 53,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Derek Fisher",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Derik Queen",
    "type": "jump_shot",
    "rating": 66,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Derrick Favors",
    "type": "jump_shot",
    "rating": 56,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Derrick Jones Jr.",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Derrick Rose",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Derrick White",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Desmond Bane",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Devin Carter",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Devin Vassell",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Dillon Brooks",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Dillon Jones",
    "type": "jump_shot",
    "rating": 66,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Dirk Nowitzki",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Domantas Sabonis",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Dominick Barlow",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Dominique Wilkins",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Donovan Clingan",
    "type": "jump_shot",
    "rating": 67,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Donovan Mitchell",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Donte DiVincenzo",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Dorian Finney-Smith",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Doug McDermott",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Drake Powell",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Draymond Green",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Drew Eubanks",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Duncan Robinson",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Duop Reath",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Dwight Powell",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Dwyane Wade",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Dylan Harper",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Dylan Windler",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Egor Demin",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Elfrid Payton",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Elton Brand",
    "type": "jump_shot",
    "rating": 62,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Enrique Freeman",
    "type": "jump_shot",
    "rating": 64,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Eric Bledsoe",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Eric Gordon",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Eugene Omoruyi",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Evan Mobley",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Frank Kaminsky III",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Franz Wagner",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Fred VanVleet",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "G.G. Jackson",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Gabe Vincent",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Garrett Temple",
    "type": "jump_shot",
    "rating": 64,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Garrison Mathews",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Gary Harris",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Gary Payton II",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Gary Trent Jr.",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "George Hill",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Georges Niang",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Giannis Antetokounmpo",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Glenn Robinson III",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Goga Bitadze",
    "type": "jump_shot",
    "rating": 67,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Goran Dragic",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Gordon Hayward",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Gorgui Dieng",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Grant Williams",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Grayson Allen",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Guerschon Yabusele",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Guis Santos",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Harrison Ingram",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Hassan Whiteside",
    "type": "jump_shot",
    "rating": 44,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Haywood Highsmith",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Herbert Jones",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Hugo Gonzalez",
    "type": "jump_shot",
    "rating": 69,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Hunter Tyson",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Immanuel Quickley",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Isaac Jones",
    "type": "jump_shot",
    "rating": 49,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Isaac Okoro",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Isaiah Collier",
    "type": "jump_shot",
    "rating": 66,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Isaiah Crawford",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Isaiah Hartenstein",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Isaiah Jackson",
    "type": "jump_shot",
    "rating": 62,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Isaiah Joe",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Isaiah Livers",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Isaiah Stewart",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Isaiah Thomas",
    "type": "jump_shot",
    "rating": 87,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Ivica Zubac",
    "type": "jump_shot",
    "rating": 45,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "J. Cole",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "J.R. Smith",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ja Morant",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jabari Smith Jr.",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Jabari Walker",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jaden Hardy",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jaden Ivey",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jaden McDaniels",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jaden Springer",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jae Crowder",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jae'Sean Tate",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jaime Jaquez Jr.",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jake LaRavia",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jakob Poeltl",
    "type": "jump_shot",
    "rating": 62,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Jakobe Walter",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Jalen Bridges",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jalen Brunson",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jalen Duren",
    "type": "jump_shot",
    "rating": 42,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Jalen Green",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Jalen Hood-Schifino",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jalen Johnson",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jalen McDaniels",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jalen Pickett",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jalen Smith",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Jalen Suggs",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Jalen Williams",
    "type": "jump_shot",
    "rating": 87,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jalen Wilson",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jamal Murray",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jamal Shead",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "James Harden",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "James Johnson",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "James Wiseman",
    "type": "jump_shot",
    "rating": 61,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Jamison Battle",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jarace Walker",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jared Butler",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jared McCain",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jaren Jackson Jr.",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Jarred Vanderbilt",
    "type": "jump_shot",
    "rating": 65,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Jarrett Allen",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Jase Richardson",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jason Kidd",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jason Preston",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jason Williams",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Javonte Green",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jaxson Hayes",
    "type": "jump_shot",
    "rating": 66,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Jay Huff",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Jaylen Brown",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jaylen Nowell",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jaylen Wells",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jaylin Williams",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Jaylon Tyson",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jayson Tatum",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "JD Davison",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jeff Green",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jerami Grant",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jeremiah Fears",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jeremiah Robinson-Earl",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Jeremy Sochan",
    "type": "jump_shot",
    "rating": 56,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jerry West",
    "type": "jump_shot",
    "rating": 90,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jett Howard",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jevon Carter",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jimmy Butler",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "JJ Redick",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Joakim Noah",
    "type": "jump_shot",
    "rating": 40,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Joan Beringer",
    "type": "jump_shot",
    "rating": 43,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Jock Landale",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Joe Harris",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Joe Ingles",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Joel Embiid",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "John Collins",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "John Konchar",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "John Stockton",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "John Wall",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Johnny Davis",
    "type": "jump_shot",
    "rating": 69,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Johnny Furphy",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Johnny Juzang",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jonas Valanciunas",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Jonathan Isaac",
    "type": "jump_shot",
    "rating": 56,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Jonathan Kuminga",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jonathan Mogbo",
    "type": "jump_shot",
    "rating": 61,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jordan Clarkson",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jordan Goodwin",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jordan Hawkins",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jordan McLaughlin",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jordan Poole",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jordan Walsh",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Jose Alvarado",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Josh Christopher",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Josh Giddey",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Josh Green",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Josh Hart",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Josh Okogie",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Josh Richardson",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jrue Holiday",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "JT Thor",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Julian Champagnie",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Julian Phillips",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Julian Strawther",
    "type": "jump_shot",
    "rating": 67,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Julius Randle",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Justin Champagnie",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Justin Edwards",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Justin Minaya",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jusuf Nurkic",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kai Jones",
    "type": "jump_shot",
    "rating": 70,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kareem Abdul-Jabbar",
    "type": "jump_shot",
    "rating": 46,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Karl Malone",
    "type": "jump_shot",
    "rating": 67,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Karl-Anthony Towns",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kasparas Jakucionis",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kawhi Leonard",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Keegan Murray",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Keldon Johnson",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kelel Ware",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kelly Olynyk",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kelly Oubre Jr.",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kemba Walker",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Kenrich Williams",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kentavious Caldwell-Pope",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kenyon Martin Jr.",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Keon Ellis",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Keon Johnson",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Kevin Duckworth",
    "type": "jump_shot",
    "rating": 55,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kevin Durant",
    "type": "jump_shot",
    "rating": 89,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Kevin Garnett",
    "type": "jump_shot",
    "rating": 70,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kevin Huerter",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kevin Knox II",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kevin Love",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Kevin Martin",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kevin Porter Jr.",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Kevon Looney",
    "type": "jump_shot",
    "rating": 51,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Keyonte George",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Khaman Maluach",
    "type": "jump_shot",
    "rating": 53,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Khris Middleton",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Killian Hayes",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "KJ SImpson",
    "type": "jump_shot",
    "rating": 54,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Klay Thompson",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kobe Brown",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kobe Bufkin",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Kon Knueppel",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kris Dunn",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Kris Murray",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kristaps Porzingis",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kyle Anderson",
    "type": "jump_shot",
    "rating": 63,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kyle Korver",
    "type": "jump_shot",
    "rating": 90,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kyle Kuzma",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kyle Lowry",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Kyrie Irving",
    "type": "jump_shot",
    "rating": 89,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Kyshawn George",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "L. Shooter",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "Lamar Odom",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Lamar Stevens",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "LaMarcus Aldridge",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "LaMelo Ball",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Landry Shamet",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Larry Bird",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Larry Nance Jr.",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Lauri Markkanen",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "LD2K",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "LeBron James",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Lester Quinones",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Liam McNeeley",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Lindy Waters III",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Lonnie Walker IV",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Lonzo Ball",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Louis Williams",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Luguentz Dort",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Luka Doncic",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Luke Kennard",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Luke Kornet",
    "type": "jump_shot",
    "rating": 63,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Magic Johnson",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Malachi Flynn",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Malaki Branham",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Malcolm Brogdon",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Malik Beasley",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Malik Monk",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Marc Gasol",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Marcus Sasser",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Marcus Smart",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "MarJon Beauchamp",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Mark Williams",
    "type": "jump_shot",
    "rating": 43,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Markelle Fultz",
    "type": "jump_shot",
    "rating": 67,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Markieff Morris",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Marquese Chriss",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Marvin Bagley III",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Mason Jones",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Mason Plumlee",
    "type": "jump_shot",
    "rating": 57,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Matas Buzelis",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Matisse Thybulle",
    "type": "jump_shot",
    "rating": 62,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Matt Ryan",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Matthew Dellavedova",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Max Christie",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Max Strus",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Maxi Kleber",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Micah Potter",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Michael Carter-Williams",
    "type": "jump_shot",
    "rating": 57,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Michael Jordan",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Michael Porter Jr.",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Mikal Bridges",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Mike Conley",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Mike Muscala",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Miles Bridges",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Miles McBride",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Mitchell Robinson",
    "type": "jump_shot",
    "rating": 46,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Monte Morris",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Moritz Wagner",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Moses Moody",
    "type": "jump_shot",
    "rating": 55,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Myles Turner",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nah'Shon Hyland",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Naji Marshall",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Nate Robinson",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Naz Reid",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Neemias Queta",
    "type": "jump_shot",
    "rating": 41,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nemanja Bjelica",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Nerlens Noel",
    "type": "jump_shot",
    "rating": 43,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nick Richards",
    "type": "jump_shot",
    "rating": 42,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nick Smith Jr.",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Nickeil Alexander-Walker",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Nicolas Batum",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Nicolas Claxton",
    "type": "jump_shot",
    "rating": 60,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nikola Jokic",
    "type": "jump_shot",
    "rating": 85,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nikola Jovic",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Nikola Topic",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Nikola Vucevic",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nique Clifford",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Noa Essengue",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Noah Clowney",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nolan Traore",
    "type": "jump_shot",
    "rating": 69,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Norman Powell",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Obi Toppin",
    "type": "jump_shot",
    "rating": 69,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ochai Agbaji",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "OG Anunoby",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Oliver-Maxence Prosper",
    "type": "jump_shot",
    "rating": 70,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Onyeka Okongwu",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Orlando Robinson",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Oshae Brissett",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Oso Ighodaro",
    "type": "jump_shot",
    "rating": 43,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Otto Porter",
    "type": "jump_shot",
    "rating": 87,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ousmane Dieng",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "P.J. Dozier",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Pacome Dadiet",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Paolo Banchero",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Pascal Siakam",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Pat Connaughton",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Patrick Baldwin Jr.",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Patrick Ewing",
    "type": "jump_shot",
    "rating": 60,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Patrick Williams",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Pau Gasol",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Paul George",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Paul Millsap",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Paul Pierce",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Paul Reed",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Payton Pritchard",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Pelle Larsson",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Penny Hardaway",
    "type": "jump_shot",
    "rating": 80,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Peyton Watson",
    "type": "jump_shot",
    "rating": 64,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "PJ Tucker",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "PJ Washington",
    "type": "jump_shot",
    "rating": 70,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Precious Achiuwa",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Quentin Grimes",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Quinten Post",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Rajon Rondo",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Rashard Lewis",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Raul Neto",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Ray Allen",
    "type": "jump_shot",
    "rating": 88,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Rayan Rupert",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Reece Beekman",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Reed Sheppard",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Reggie Bullock",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Richaun Holmes",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Ricky Council IV",
    "type": "jump_shot",
    "rating": 65,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "RJ Barrett",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Robert Covington",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Robert Dillingham",
    "type": "jump_shot",
    "rating": 69,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Robert Williams III",
    "type": "jump_shot",
    "rating": 44,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Ron Holland",
    "type": "jump_shot",
    "rating": 61,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ronnie 2k",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Royce O'Neale",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Rudy Gay",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Rudy Gobert",
    "type": "jump_shot",
    "rating": 41,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Rui Hachimura",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Russell Westbrook",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Ryan Dunn",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ryan Rollins",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Saddiq Bey",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Sam Hauser",
    "type": "jump_shot",
    "rating": 88,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Sam Merrill",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Sam Perkins",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Sandro Mamukelashvili",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Santi Aldama",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Scoot Henderson",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Scottie Barnes",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Scottie Pippen",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Scotty Pippen Jr.",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Serge Ibaka",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Seth Curry",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Shaedon Sharpe",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Shai Gilgeous-Alexander",
    "type": "jump_shot",
    "rating": 87,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Shake Milton",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Shaquille O'Neal",
    "type": "jump_shot",
    "rating": 38,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Shawn Marion",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Simone Fontecchio",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Skal Labissiere",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Spencer Dinwiddie",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Stanley Johnson",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Stephen Curry",
    "type": "jump_shot",
    "rating": 93,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Stephon Castle",
    "type": "jump_shot",
    "rating": 68,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Steve Francis",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Steve Kerr",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Steve Nash",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Steven Adams",
    "type": "jump_shot",
    "rating": 41,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Sviatoslav Mykhailiuk",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Taj Gibson",
    "type": "jump_shot",
    "rating": 46,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Talen Horton-Tucker",
    "type": "jump_shot",
    "rating": 55,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Tari Eason",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Taurean Prince",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Taylor Hendricks",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Taze Moore",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Terance Mann",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Terence Davis",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Terrence Ross",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Terrence Shannon Jr.",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Terry Rozier III",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Terry Taylor",
    "type": "jump_shot",
    "rating": 73,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Thaddeus Young",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Thomas Bryant",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Thomas Sorber",
    "type": "jump_shot",
    "rating": 64,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Tidjane Salaun",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Tim Duncan",
    "type": "jump_shot",
    "rating": 45,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "TJ McConnell",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "TJ Warren",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Tobias Harris",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Toni Kukoc",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Tony Bradley",
    "type": "jump_shot",
    "rating": 47,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Tony Parker",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Tony Snell",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Torrey Craig",
    "type": "jump_shot",
    "rating": 70,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Toumani Camara",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Trae Young",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Tre Johnson",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Tre Jones",
    "type": "jump_shot",
    "rating": 75,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Tre Mann",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Trendon Watford",
    "type": "jump_shot",
    "rating": 74,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Trevelin Queen",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Trevor Ariza",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Trey Burke",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Trey Lyles",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Trey Murphy III",
    "type": "jump_shot",
    "rating": 82,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Tristan da Silva",
    "type": "jump_shot",
    "rating": 57,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Tristan Thompson",
    "type": "jump_shot",
    "rating": 47,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Tristen Newton",
    "type": "jump_shot",
    "rating": 72,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Ty Jerome",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Tyler Herro",
    "type": "jump_shot",
    "rating": 86,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Tyler Kolek",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Tyler Smith",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Tyrese Haliburton",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Tyrese Maxey",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "TyTy Washington Jr.",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Tyus Jones",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Udonis Haslem",
    "type": "jump_shot",
    "rating": 52,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "V.J. Edgecombe",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Victor Wembanyama",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Vince Carter",
    "type": "jump_shot",
    "rating": 79,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Vince Williams",
    "type": "jump_shot",
    "rating": 62,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Vladimir Radmanovic",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Vlatko Cancar",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Walker Kessler",
    "type": "jump_shot",
    "rating": 56,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Walter Clayton Jr.",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Wayne Ellington",
    "type": "jump_shot",
    "rating": 83,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Wendell Moore Jr.",
    "type": "jump_shot",
    "rating": 77,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Will Hernangomez",
    "type": "jump_shot",
    "rating": 69,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Will Riley",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Xavier Tillman",
    "type": "jump_shot",
    "rating": 60,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Yang Hansen",
    "type": "jump_shot",
    "rating": 67,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Yanic Konan Niederhauser",
    "type": "jump_shot",
    "rating": 47,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Yves Missi",
    "type": "jump_shot",
    "rating": 48,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Zaccharie Risacher",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Zach Collins",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Zach Edey",
    "type": "jump_shot",
    "rating": 71,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Zach LaVine",
    "type": "jump_shot",
    "rating": 84,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Zach Randolph",
    "type": "jump_shot",
    "rating": 67,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Zeke Nnaji",
    "type": "jump_shot",
    "rating": 78,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Ziaire Williams",
    "type": "jump_shot",
    "rating": 76,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Zion Williamson",
    "type": "jump_shot",
    "rating": 81,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Aaron Gordon",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'10"
  },
  {
    "name": "Anthony Davis",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Boban Marjanovic",
    "type": "post_fade",
    "rating": null,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "Brandon Ingram",
    "type": "post_fade",
    "rating": null,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "De'Aaron Fox",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "DeMar DeRozan",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Devin Booker",
    "type": "post_fade",
    "rating": null,
    "height": "Between 5'9 and 6'9"
  },
  {
    "name": "Dirk Nowitzki",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Domantas Sabonis",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Donovan Mitchel",
    "type": "post_fade",
    "rating": null,
    "height": "Between 5'9 and 6'4"
  },
  {
    "name": "Harrison Barnes",
    "type": "post_fade",
    "rating": null,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "Jayson Tatum",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Jonathan Kuminga",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Karl Malone",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Karl-Anthony Towns",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Kevin Durant",
    "type": "post_fade",
    "rating": null,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "Kevin Huerter",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Kobe Bryant",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "LeBron James",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Luka Doncic",
    "type": "post_fade",
    "rating": null,
    "height": "Between 5'9 and 7'4"
  },
  {
    "name": "Michael Jordan",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'9"
  },
  {
    "name": "Nikola Jokic",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Nikola Vucevic",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'10 and 7'4"
  },
  {
    "name": "Pascal Siakam",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 7'4"
  },
  {
    "name": "Paul George",
    "type": "post_fade",
    "rating": null,
    "height": "Between 6'5 and 6'9"
  }
];

const TYPE_LABELS = {
  jump_shot: "Jump Shot",
  go_to: "Go-To Shot",
  dribble_pullup: "Dribble Pull-Up",
  post_fade: "Post Fade",
};

const VISUAL_CUES = [
  { name: "Set Point",   offset: 0.0,   note: "Release when the shot reaches its set point." },
  { name: "Apex",        offset: 0.12,  note: "Release at the top of the jump." },
  { name: "Push",        offset: -0.08, note: "Release as the hands push forward." },
  { name: "Wrist Flick", offset: 0.06,  note: "Release on the wrist flick." }
];

/* In-game Release Speed slider — 4 discrete notches (not 0–100). */
const RELEASE_SPEEDS = [
  { label: "Slow",       factor: 25 },
  { label: "Normal",     factor: 50 },
  { label: "Quick",      factor: 75 },
  { label: "Very Quick", factor: 100 }
];

const DEFAULT_RELEASE_SPEED_INDEX = 1;

/* Estimated timing scale per package type (not published by 2K). */
const TYPE_TIMING = {
  jump_shot: 1.0,
  dribble_pullup: 0.95,
  post_fade: 1.1
};

/* Go-To lab timings — NBA2KLab moving-jumpers, Turbo=No, Main hand. */
const GO_TO_LAB = [
  {
    "jumper": "Booker",
    "early_ms": 742,
    "late_ms": 750,
    "release_ms": 746,
    "window_ms": 8
  },
  {
    "jumper": "Durant",
    "early_ms": 941,
    "late_ms": 970,
    "release_ms": 956,
    "window_ms": 29
  },
  {
    "jumper": "Huerter",
    "early_ms": 872,
    "late_ms": 880,
    "release_ms": 876,
    "window_ms": 8
  },
  {
    "jumper": "Kobe",
    "early_ms": 761,
    "late_ms": 780,
    "release_ms": 770,
    "window_ms": 19
  }
];

/* Fallback when no lab row: standing jump estimate * ratio + gather. */
const GO_TO_ESTIMATE = { ratio: 1.52, gather_ms: 165, window_scale: 0.62 };

/*
 * Researched green windows (ms) at Normal release speed + Set Point cue.
 * Sources: NBA2KLab moving-jumper PGW, 2K26 meta guides (AOEAH, community).
 * Used for best-shot ranking; non-listed animations use the estimate formula.
 */
const GREEN_WINDOW_META = {
  "Charles Bassey": { window_ms: 68, note: "2026 meta — largest tested window" },
  "Dirk Nowitzki": { window_ms: 64, note: "Big man high-release staple" },
  "David Robinson": { window_ms: 61, note: "Forgiving stretch big" },
  "Kevin Durant": { window_ms: 58, note: "Tall wing release" },
  "Cameron Thomas": { window_ms: 56, note: "Guard meta blend" },
  "Collin Sexton": { window_ms: 55, note: "Catch-and-shoot guard" },
  "Stephen Curry": { window_ms: 54, note: "High 3PT rating builds" },
  "Kyle Korver": { window_ms: 53, note: "Wing release" },
  "Mike Muscala": { window_ms: 52, note: "Stretch big" }
};

/*
 * Researched custom jumper recipes — copy into MyPlayer → Jumpshot Creator.
 * height_min/max in inches. min_3pt/max_3pt = recommended 3PT rating band.
 * blend = % toward Upper Release 1 (0–100). stats = in-game Set Stats scores.
 */
const CUSTOM_BUILDS = [
  {
    id: "guard_thomas_max",
    label: "Thomas Max Window",
    height_min: 69,
    height_max: 76,
    min_3pt: 38,
    max_3pt: 99,
    base: "Cameron Thomas",
    release_1: "Cameron Thomas",
    release_2: "Cameron Thomas",
    blend: 100,
    release_speed: 3,
    visual_cue: 0,
    window_ms: 58,
    stats: { release_height: 78, defense_immunity: 84, timing_stability: 91, release_speed: 94 },
    note: "Best green window under 6'5 — 100% Thomas, max release speed"
  },
  {
    id: "guard_curry_sexton",
    label: "Curry / Sexton Quick",
    height_min: 69,
    height_max: 76,
    min_3pt: 90,
    max_3pt: 99,
    base: "Stephen Curry",
    release_1: "Collin Sexton",
    release_2: "Collin Sexton",
    blend: 100,
    release_speed: 3,
    visual_cue: 0,
    window_ms: 55,
    stats: { release_height: 80, defense_immunity: 88, timing_stability: 86, release_speed: 96 },
    note: "High 3PT guards — Curry base, 100% Sexton releases, very quick"
  },
  {
    id: "guard_sexton_blend",
    label: "Sexton / Thomas Blend",
    height_min: 69,
    height_max: 76,
    min_3pt: 74,
    max_3pt: 89,
    base: "Collin Sexton",
    release_1: "Cameron Thomas",
    release_2: "Collin Sexton",
    blend: 65,
    release_speed: 3,
    visual_cue: 0,
    window_ms: 54,
    stats: { release_height: 79, defense_immunity: 86, timing_stability: 84, release_speed: 93 },
    note: "Mid 3PT guards — Thomas release mix for defensive immunity"
  },
  {
    id: "wing_bassey_max",
    label: "Bassey Max Window",
    height_min: 77,
    height_max: 88,
    min_3pt: 38,
    max_3pt: 99,
    base: "Charles Bassey",
    release_1: "Charles Bassey",
    release_2: "Charles Bassey",
    blend: 100,
    release_speed: 3,
    visual_cue: 0,
    window_ms: 68,
    stats: { release_height: 82, defense_immunity: 86, timing_stability: 95, release_speed: 97 },
    note: "2026 #1 meta — 100% Bassey, max speed, largest tested green window"
  },
  {
    id: "wing_kd_luka",
    label: "KD / Luka High Release",
    height_min: 77,
    height_max: 81,
    min_3pt: 89,
    max_3pt: 99,
    base: "Kevin Durant",
    release_1: "Luka Doncic",
    release_2: "Kevin Durant",
    blend: 70,
    release_speed: 3,
    visual_cue: 0,
    window_ms: 56,
    stats: { release_height: 94, defense_immunity: 90, timing_stability: 84, release_speed: 92 },
    note: "Tall wings 6'5–6'9 — 70% Luka / 30% KD blend, hard to contest"
  },
  {
    id: "wing_korver",
    label: "Korver Wing Release",
    height_min: 77,
    height_max: 81,
    min_3pt: 84,
    max_3pt: 88,
    base: "Kyle Korver",
    release_1: "Kyle Korver",
    release_2: "Collin Sexton",
    blend: 81,
    release_speed: 2,
    visual_cue: 0,
    window_ms: 52,
    stats: { release_height: 86, defense_immunity: 82, timing_stability: 80, release_speed: 78 },
    note: "Balanced wing jumper — Korver-heavy blend"
  },
  {
    id: "big_kd_west",
    label: "KD / West Stretch",
    height_min: 82,
    height_max: 88,
    min_3pt: 88,
    max_3pt: 99,
    base: "Kevin Durant",
    release_1: "Kevin Durant",
    release_2: "David West",
    blend: 76,
    release_speed: 3,
    visual_cue: 0,
    window_ms: 57,
    stats: { release_height: 92, defense_immunity: 88, timing_stability: 82, release_speed: 90 },
    note: "High 3PT stretch bigs — 76% KD / 24% West"
  },
  {
    id: "big_cousins_collins",
    label: "Cousins / Collins Pop",
    height_min: 82,
    height_max: 88,
    min_3pt: 80,
    max_3pt: 87,
    base: "DeMarcus Cousins",
    release_1: "DeMarcus Cousins",
    release_2: "John Collins",
    blend: 50,
    release_speed: 3,
    visual_cue: 0,
    window_ms: 54,
    stats: { release_height: 86, defense_immunity: 80, timing_stability: 78, release_speed: 88 },
    note: "Pick-and-pop bigs — 50/50 Cousins/Collins"
  },
  {
    id: "big_dirk",
    label: "Dirk High Release",
    height_min: 82,
    height_max: 88,
    min_3pt: 67,
    max_3pt: 79,
    base: "Dirk Nowitzki",
    release_1: "Dirk Nowitzki",
    release_2: "Mike Muscala",
    blend: 100,
    release_speed: 2,
    visual_cue: 0,
    window_ms: 60,
    stats: { release_height: 90, defense_immunity: 78, timing_stability: 86, release_speed: 72 },
    note: "Mid 3PT bigs — classic Dirk release, quick speed"
  },
  {
    id: "big_robinson_pure",
    label: "Robinson Pure",
    height_min: 82,
    height_max: 88,
    min_3pt: 38,
    max_3pt: 62,
    base: "David Robinson",
    release_1: "David Robinson",
    release_2: "David Robinson",
    blend: 100,
    release_speed: 1,
    visual_cue: 0,
    window_ms: 58,
    stats: { release_height: 76, defense_immunity: 72, timing_stability: 88, release_speed: 58 },
    note: "Very low 3PT bigs — 100% Robinson, normal speed, easy unlocks"
  },
  {
    id: "big_robinson_muscala",
    label: "Robinson / Muscala Forgiving",
    height_min: 82,
    height_max: 88,
    min_3pt: 67,
    max_3pt: 83,
    base: "David Robinson",
    release_1: "Mike Muscala",
    release_2: "David West",
    blend: 86,
    release_speed: 1,
    visual_cue: 0,
    window_ms: 62,
    stats: { release_height: 80, defense_immunity: 76, timing_stability: 92, release_speed: 62 },
    note: "Mid-low 3PT bigs — 86% Muscala / 14% West, normal speed"
  }
];
